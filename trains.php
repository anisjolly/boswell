...
foreach ($text->GetStationBoardResult->trainServices->service as $service) {
  ...
  echo '    <div class="service-detail" data-shown="false">'.chr(10);
//  echo '      <p>Platform: '.($service->platform=='' ? 'TBC' : $service->platform).'<span class="pull-right">'.$service->operator.'</span></p>'.chr(10);
  echo '    </div>'.chr(10);
  echo '  </div>'.chr(10);
  echo '</div>'.chr(10);
